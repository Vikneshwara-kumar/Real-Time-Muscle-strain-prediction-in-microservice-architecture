...
class Extractor:
    ...
    def downsample(self, df):
        ...
        try:
            ...
            # Resample the signal
            resampled_df = df.values

            downsampled_df = pd.DataFrame(resampled_df, columns=df.columns)
    ...
    def compute_PSD(self, windows):
        ...
        for window in windows:
            ...
            f, psd = welch(window, fs=self.fs, nperseg=30)
    ...
    def compute_stft(self, windows):
        ...
        for window in windows:
            ...
            f_stft, t_stft, Zxx = stft(window, fs=self.fs, nperseg=30)
