...
6. To configure MQTT client open `/AeroPy/DataManager.py` change the the mqtt broker address, port, and topic. If you need more topics add the new topic below the default topic.
```python
class DataKernel():
...
``` 
7. To add more users or sensors add the `self.collected_data_array = []` below the default array, and configure 2nd sensor by copying the below sniped and renaming it with sufix 2. Paste the copied one in `def processData(self, data_queue):` function in the  `/AeroPy/DataManager.py` script.
```python
...
```
8. Make sure the Trigno base station or lite are plugged in, then Run `DelsysPythonDemo.py`
...
To start the program again close the Data Collection window and run `DelsysPythonDemo.py`
