...
NORMALIZED_DATA_TOPIC = 'user2_extracted_features'
PREDICTION_TOPIC = 'predictions_02'

consumer = KafkaConsumer(
    NORMALIZED_DATA_TOPIC,
    bootstrap_servers=[KAFKA_BROKER],
    group_id='102', # Specify the consumer group ID
...
def consume_and_process():
    ...
    for message in consumer:
        ...
        if len(df_ary)>=30:
            ...
            #d = {'Prediction': result, 'Inference Latency': Infer_latency,'Request Latency': requect_latency,'CPU Usage':cpu_usage,'Memory Usage':memory_usage,'throughput':throughtput}
            #final_out =pd.DataFrame(data=d)
            #df_json_str = final_out.to_json(df_json_str)
            #df_json_obj = json.loads(df_json_str)

            #producer.send("LSTM_Result_Moicroservice_lstm", value=df_json_obj)
